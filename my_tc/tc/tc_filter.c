...
{
	// MATTEO modificato, memoria stack non sufficente! serve puntatore e allocazion in heap
	struct {
		struct nlmsghdr 	n;
		struct tcmsg 		t;
		char   			buf[MAX_MSG];
	...
	if (q) {
		...
		if (fhandle) {
			printk(KERN_DEBUG "[MTC] Must specify filter type when using \"handle\"\n");
			return -1;
		}
		if (argc) {
			if (matches(*argv, "help") == 0)
				usage();
			printk(KERN_DEBUG "[MTC] Garbage instead of arguments \"%s ...\". Try \"tc filter help\".\n", *argv);
			return -1;
		}
	}
	if (est.ewma_log)
...
{
	printk(KERN_DEBUG "[MAT] tc_filter.c | do_filter()\n");
	if (argc < 1)
		return -1;
...
*/
	...
	printk(KERN_DEBUG "Command \"%s\" is unknown, try \"tc filter help\".\n", *argv);
	return -1;
}
