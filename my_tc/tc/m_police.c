...
{
	printk(KERN_DEBUG "Usage: ... police rate BPS burst BYTES[/BYTES] [ mtu BYTES[/BYTES] ]\n");
	printk(KERN_DEBUG "                [ peakrate BPS ] [ avrate BPS ] [ overhead BYTES ]\n");
	printk(KERN_DEBUG "                [ linklayer TYPE ] [ ACTIONTERM ]\n");

	printk(KERN_DEBUG "Old Syntax ACTIONTERM := action <EXCEEDACT>[/NOTEXCEEDACT] \n");
	printk(KERN_DEBUG "New Syntax ACTIONTERM := conform-exceed <EXCEEDACT>[/NOTEXCEEDACT] \n");
	printk(KERN_DEBUG "Where: *EXCEEDACT := pipe | ok | reclassify | drop | continue \n");
	printk(KERN_DEBUG "Where:  pipe is only valid for new syntax \n");
}

static void explain1(char *arg)
{
	printk(KERN_DEBUG "Illegal \"%s\"\n", arg);
}
...
int act_parse_police(struct action_util *a,int *argc_p, char ***argv_p, int tca_id, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		...
		if (matches(*argv, "index") == 0) {
			...
			if (get_u32(&p.index, *argv, 10)) {
				printk(KERN_DEBUG "Illegal \"index\"\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (buffer) {
				printk(KERN_DEBUG "Double \"buffer/burst\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mtu) {
				printk(KERN_DEBUG "Double \"mtu/minburst\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (mpu) {
				printk(KERN_DEBUG "Double \"mpu\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (p.rate.rate) {
				printk(KERN_DEBUG "Double \"rate\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (avrate) {
				printk(KERN_DEBUG "Double \"avrate\" spec\n");
				return -1;
			}
			...
			NEXT_ARG();
			if (p.peakrate.rate) {
				printk(KERN_DEBUG "Double \"peakrate\" spec\n");
				return -1;
			}
			...
			if (get_police_result(&p.action, &presult, *argv)) {
				printk(KERN_DEBUG "Illegal \"action\"\n");
				return -1;
			}
	...
	if (p.rate.rate && !buffer) {
		printk(KERN_DEBUG "\"burst\" requires \"rate\".\n");
		return -1;
	}
	if (p.peakrate.rate) {
		if (!p.rate.rate) {
			printk(KERN_DEBUG "\"peakrate\" requires \"rate\".\n");
			return -1;
		}
		if (!mtu) {
			printk(KERN_DEBUG "\"mtu\" is required, if \"peakrate\" is requested.\n");
			return -1;
		}
	}
	...
	if (p.rate.rate) {
		...
		if (tc_calc_rtable(&p.rate, rtab, Rcell_log, mtu, linklayer) < 0) {
			printk(KERN_DEBUG "TBF: failed to calculate rate table.\n");
			return -1;
		}
	...
	if (p.peakrate.rate) {
		...
		if (tc_calc_rtable(&p.peakrate, ptab, Pcell_log, mtu, linklayer) < 0) {
			printk(KERN_DEBUG "POLICE: failed to calculate peak rate table.\n");
			return -1;
		}
	}
