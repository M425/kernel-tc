...
{
	//MTC my flag is MTC_OWNER-->
	unsigned short * mat_arg = (unsigned short *) arg;
	//<--
	
	//MTC in mtc net is not passed -->
	struct net *net = NULL;
	//<--
...
// 	struct net *net = sock_net(skb->sk);
	...
	//MTC just log -->
	if(*mat_arg == MATTEO_M_OWNER)
		printk(KERN_DEBUG "[MTC] [tc_get_qdisc] it's me\n");
	//<--
	...
	//MTC change the way retrieve dev -->
	if(*mat_arg == MATTEO_M_OWNER)
		dev = skb->dev;
	else
	//<--
...
int tc_modify_qdisc(struct sk_buff *skb, struct nlmsghdr *n, void *arg)
{
	...
	//MTC just log -->
	if(*mat_arg == MATTEO_M_OWNER)
		printk(KERN_DEBUG "[MTC] [tc_modify_qdisc] it's me\n");
	//<--
...
replay:
	...
	else if (tca[TCA_KIND] && nla_strcmp(tca[TCA_KIND], q->ops->id)) {
		return -EINVAL;
	}
	else {
	}
...
int tc_ctl_tclass(struct sk_buff *skb, struct nlmsghdr *n, void *arg)
{
	...
	//MTC just log -->
	if(*mat_arg == MATTEO_M_OWNER)
		printk(KERN_DEBUG "[MTC] [tc_ctl_tclass] it's me\n");
	//<--
